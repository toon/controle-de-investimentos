  ...
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title><v-icon>mdi-cog</v-icon> Tipos de Agrupamentos de Ativo</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
...
export default {
  ...
  methods: {
    ...
    loadItems() {
      api.get("/tipoativoagrupamentos").then((response) => {
    ...
    deleteItemConfirm () {
      api.delete(`/tipoativoagrupamento/${this.editedItem.id}`).then(() => 
        this.loadItems(),
        this.closeDelete()
      );
    },
    ...
    save () {
      if (this.editedIndex > -1) {
        api.put(`/tipoativoagrupamento/${this.editedItem.id}`, this.editedItem).then(() => 
          this.loadItems()
        )
      } else {
        api.post("/tipoativoagrupamento", this.editedItem).then(() => 
          this.loadItems()
        )
      }
      this.close()
