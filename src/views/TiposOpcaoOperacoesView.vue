  ...
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title><v-icon>mdi-cog</v-icon> Tipos de Operação em Opção</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
                ...
                <v-row>
                  ...
                  >
                    <v-text-field
                      ...
                      label="Tipo de Operação em Opção"
                    ></v-text-field>
                  </v-col>
                  <v-col
                    cols="12"
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { title: "Tipo de Operação em Opção", key:"nome", value: "nome" },
  ...
  methods: {
    ...
    loadItems() {
      api.get("/tipoopcaooperacao").then((response) => {
    ...
    deleteItemConfirm () {
      api.delete(`/tipoopcaooperacao/${this.editedItem.id}`).then(() => 
        this.loadItems(),
        this.closeDelete()
      );
    },
    ...
    save () {
      if (this.editedIndex > -1) {
        api.put(`/tipoopcaooperacao/${this.editedItem.id}`, this.editedItem).then(() => 
          this.loadItems()
        )
      } else {
        api.post("/tipoopcaooperacao", this.editedItem).then(() => 
          this.loadItems()
        )
      }
      this.close()
