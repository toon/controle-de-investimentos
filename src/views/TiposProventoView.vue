  ...
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title><v-icon>mdi-cog</v-icon> Tipos de Provento</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { title: "Tipo de Provento", key:"nome", value: "nome" },
  ...
  methods: {
    ...
    deleteItemConfirm () {
      api.delete(`/tipoprovento/${this.editedItem.id}`).then(() => 
        this.loadItems(),
        this.closeDelete()
      );
    },
    ...
    save () {
      if (this.editedIndex > -1) {
        api.put(`/tipoprovento/${this.editedItem.id}`, this.editedItem).then(() => 
          this.loadItems()
        )
      } else {
        api.post("/tipoprovento", this.editedItem).then(() => 
          this.loadItems()
        )
      }
      this.close()
