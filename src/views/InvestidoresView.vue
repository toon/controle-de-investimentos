...
<template>
  ...
  >
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title>
          <v-icon>mdi-account-group</v-icon> Investidores
        </v-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm() {
      api.delete(`/investidor/${this.editedItem.id}`)
        .then(() => {
          this.loadItems()
          this.closeDelete()
        })
      ...
      this.$refs.form.validate().then((formResult) => {
        if (formResult.valid) {
          if (this.editedIndex > -1) {
            api.put(`/investidor/${this.editedItem.id}`, this.editedItem)
              .then(() => {
                this.loadItems()
                this.close()
              })
            ...
            api.post('/investidor', this.editedItem)
              .then(() => {
                this.loadItems()
                this.close()
              })
