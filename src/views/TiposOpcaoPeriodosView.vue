  ...
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title><v-icon>mdi-cog</v-icon> Tipos de Período em Opção</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
                ...
                <v-row>
                  ...
                  >
                    <v-text-field
                      ...
                      label="Tipo de Período em Opção"
                    ></v-text-field>
                  </v-col>
                  <v-col
                    cols="12"
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { title: "Tipo de Período em Opção", key:"nome", value: "nome" },
  ...
  methods: {
    ...
    loadItems() {
      api.get("/tipoopcaoperiodo").then((response) => {
    ...
    deleteItemConfirm () {
      api.delete(`/tipoopcaoperiodo/${this.editedItem.id}`).then(() => 
        this.loadItems(),
        this.closeDelete()
      );
    },
    ...
    save () {
      if (this.editedIndex > -1) {
        api.put(`/tipoopcaoperiodo/${this.editedItem.id}`, this.editedItem).then(() => 
          this.loadItems()
        )
      } else {
        api.post("/tipoopcaoperiodo", this.editedItem).then(() => 
          this.loadItems()
        )
      }
      this.close()
