  ...
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title><v-icon>mdi-cog</v-icon> Tipos de Status em Opção</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { title: "Tipo de Status em Opção", key:"nome", value: "nome" },
  ...
  methods: {
    ...
    deleteItemConfirm () {
      api.delete(`/tipoopcaostatus/${this.editedItem.id}`).then(() => 
        this.loadItems(),
        this.closeDelete()
      );
    },
    ...
    save () {
      if (this.editedIndex > -1) {
        api.put(`/tipoopcaostatus/${this.editedItem.id}`, this.editedItem).then(() => 
          this.loadItems()
        )
      } else {
        api.post("/tipoopcaostatus", this.editedItem).then(() => 
          this.loadItems()
        )
      }
      this.close()
