  ...
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title><v-icon>mdi-cog</v-icon> Status de Operação de Taxa</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { title: "Status de operação de taxa", key:"nome", value: "nome" },
  ...
  methods: {
    ...
    loadItems() {
      api.get("/operacaotaxastatuss").then((response) => {
    ...
    deleteItemConfirm () {
      api.delete(`/operacaotaxastatus/${this.editedItem.id}`).then(() => 
        this.loadItems(),
        this.closeDelete()
      );
    },
    ...
    save () {
      if (this.editedIndex > -1) {
        api.put(`/operacaotaxastatus/${this.editedItem.id}`, this.editedItem).then(() => 
          this.loadItems()
        )
      } else {
        api.post("/operacaotaxastatus", this.editedItem).then(() => 
          this.loadItems()
        )
      }
      this.close()
